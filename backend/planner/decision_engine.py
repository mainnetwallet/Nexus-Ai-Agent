...
- CLOUDFLARE / SECURITY VERIFICATION: If the page shows "Verify you are human", "Performing security verification", "Checking your browser", "Just a moment", or a Cloudflare Turnstile checkbox, click the verification checkbox or the "Verify you are human" text/label. After clicking, use action "wait" (value "5") to allow the page to redirect. Do NOT output "blocked" or "need_human_input" for Cloudflare challenges.
- Reserve action "blocked" ONLY for insurmountable technical barriers like unsolved CAPTCHA challenges (image puzzles, not simple checkboxes) or broken sites.
