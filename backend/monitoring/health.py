...
class HealthMonitor:
    ...
    async def _check_mcp(self) -> ComponentHealth:
        ...
        try:
            ...
            if mcp is None:
                status, detail = "unknown", "MCP Core not initialized"
            else:
                connectors = mcp.registry.list_connectors()
                connector_health = await mcp.health()
                enabled = [c["name"] for c in connectors if c["enabled"]]
                errored = [name for name, h in connector_health.items() if h.get("status") == "error"]
                if errored:
                    status, detail = "degraded", f"connectors with errors: {', '.join(errored)}"
                elif enabled:
                    status, detail = "ok", f"enabled: {', '.join(enabled)}"
                else:
                    status, detail = "degraded", "no connectors enabled"
        except Exception as exc:  # noqa: BLE001
            status, detail = "down", f"check raised: {exc}"
        latency_ms = round((time.perf_counter() - start) * 1000, 2)
        return ComponentHealth("mcp", status, detail, latency_ms)
